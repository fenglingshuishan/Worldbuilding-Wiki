...
- [x] 工作区文档规范检查无错误或警告。
- [x] wheel 和 sdist 内容检查通过，包含完整前端与示例地图资源。
- [x] wheel 在全新 `/tmp` 虚拟环境安装，版本为 `0.4.0`。
- [x] 已安装 wheel 在源码目录外启动真实服务，健康、默认示例创建、驾驶舱和地图资源接口通过。
...
- [x] 解压后的真实程序启动回环服务，创建匿名世界库、载入 13 条示例并读取驾驶舱。
...
`main` 推送后必须先等待 `Build release artifacts` 完整成功，再创建 `v0.4.0` 标签。标签工作流必须满足：
